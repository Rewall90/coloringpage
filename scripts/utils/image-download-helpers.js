    ...
    const attempt = () => {
        ...
        if (response.statusCode === 200) {
          ...
          file.on("error", (err) => {
            ...
            if (attempts < maxRetries) {
              console.log(
                `⚠️  Retry ${attempts}/${maxRetries} for ${path.basename(outputPath)}`,
              );
      ...
      request.on("error", (err) => {
        ...
        if (attempts < maxRetries) {
          ...
          reject(
            new Error(
              `Request failed after ${maxRetries} attempts: ${err.message}`,
            ),
          );
        }
      });
...
export const generateLocalImagePath = (imageContext) => {
  ...
  if (type === "homepage_category") {
    ...
    // Content images: /images/collections/animals-coloring-pages/bunny-coloring-page/bunny-coloring-page-750x1000.webp
...
export const downloadImage = async (imageContext, sanityUrl, manifest) => {
  ...
  try {
    ...
    console.error(
      `❌ Failed to download ${path.basename(fullPath)}:`,
      error.message,
    );
...
export const cleanupOrphanedImages = (currentImagePaths) => {
  ...
  const orphanedPaths = [...manifestPaths].filter(
