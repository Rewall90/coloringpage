...
const extractContentImages = (content, categorySlug, pageSlug) => {
  ...
  const coloringPageBlocks = content.filter(block => block._type === 'coloringPage');
...
export const processContentImages = async posts => {
  ...
  for (const post of posts) {
    ...
    const contentImages = extractContentImages(post.content, categorySlug, pageSlug);
...
export const processAllImages = async (categories, posts) => {
  ...
  if (allImageRequests.length === 0) {
    ...
    return { success: true, summary: { total: 0, downloaded: 0, skipped: 0, failed: 0 } };
  }
