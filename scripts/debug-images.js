...
function debugImages() {
  console.log('🔍 Image System Debug Information\n');
  ...
  if (fs.existsSync(STATIC_DIR)) {
    ...
    if (fs.existsSync(IMAGES_DIR)) {
      console.log(`   ✅ static/images/ exists`);
    } else {
      console.log(`   ❌ static/images/ missing`);
    }
  } else {
  ...
  try {
    ...
    if (Object.keys(manifest.images).length > 0) {
      ...
      recentImages.forEach(([path, info]) => {
        ...
        console.log(`     • ${fileName} (${formatBytes(info.fileSize)}) - ${downloadTime}`);
      });
  ...
  if (localScan.files.length === 0) {
    ...
    console.log(`   📁 Directories: ${localScan.directories.length}`);
    console.log(`   🖼️  Image Files: ${localScan.files.length}`);
    console.log(`   💽 Total Size: ${formatBytes(localScan.totalSize)}`);
  ...
  try {
    ...
    if (inManifestNotLocal.length > 0) {
      console.log(`   ⚠️  In manifest but missing locally: ${inManifestNotLocal.length}`);
      if (inManifestNotLocal.length <= 5) {
        inManifestNotLocal.forEach(path => console.log(`     • ${path}`));
      }
    }

    if (inLocalNotManifest.length > 0) {
      console.log(`   ⚠️  Local files not in manifest: ${inLocalNotManifest.length}`);
      if (inLocalNotManifest.length <= 5) {
        inLocalNotManifest.forEach(path => console.log(`     • ${path}`));
      }
    }

    if (inManifestNotLocal.length === 0 && inLocalNotManifest.length === 0) {
      console.log('   ✅ Manifest and local files are in sync');
    }

  } catch (error) {
    console.log(`   ❌ Error checking sync: ${error.message}`);
  }
