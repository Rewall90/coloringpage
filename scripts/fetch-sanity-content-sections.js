...
import {
  ...
  portableTextToExcerpt,
} from "./utils/portable-text-helpers.js";
...
const cleanupOldCategories = () => {
  ...
  for (const file of existingFiles) {
    try {
      ...
      if (
        content.includes("layout: main-category") ||
  ...
  for (const category of categories) {
    try {
      if (!category.title || !category.slug) {
        console.warn(
          `⚠️  Skipping category without title or slug (ID: ${category._id})`,
        );
        skipped++;
        continue;
      }
      ...
      for (const page of categoryPages) {
        try {
          if (!page.title || !page.imageUrl) {
            console.warn(
              `⚠️  Skipping incomplete coloring page "${page.title}" - missing image`,
            );
            continue;
          }
          ...
          const pageDimensions = getImageDimensions(
          ...
          console.error(
            `❌ Error processing coloring page "${page.title}":`,
            error.message,
          );
        }
      }
...
const generatePostsInSections = async () => {
  ...
  for (const post of posts) {
    try {
      ...
      const contentMarkdown = portableTextToMarkdown(
        post.content,
        assetMappings,
        pageContext,
      );
      ...
      const description =
        post.excerpt || portableTextToExcerpt(post.content, 25);

      const dimensions = getImageDimensions(
        post.heroImageDimensions,
        IMAGE_SIZES.hero,
      );
  ...
  try {
    ...
    if (!imageResult.success) {
      ...
      if (imageResult.summary.criticalFailures > 0) {
        ...
        console.error(
          "   Homepage category images are required for the site to function properly",
        );
        process.exit(1);
      } else if (imageResult.summary.failed > 0) {
        console.warn(
          `⚠️  ${imageResult.summary.failed} content images failed to download`,
        );
        console.warn(
          "   Site will continue building, but some images may be missing",
        );
      }
    } else {
